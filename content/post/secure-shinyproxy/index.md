...
title: "Securing your ShinyProxy deployment of R Shiny apps"
subtitle: ""
summary: ""
authors: [databentobox]
tags: [R, R Shiny, ShinyProxy]
categories: [R Shiny]
date: 2020-05-01T10:10:55+01:00
lastmod: 2020-05-01T10:10:55+01:00
featured: false
draft: true
...
PLACEHOLDER FOR FRAMEWORK GRAPH
...
Before we diving into the tutorial, there are a couple of things that need to be done, which I won't cover in much details here.
...
* Ubuntu 16.04 LTS (although other version may also work)
...
If you already own a domain name (e.g. example.com), it is easy to set up a subdomain (e.g. app.example.com) to host your Shiny app. You will need to set up an A record with your domain registrar and point it to the IP address of the `ShinyProxy Instance`.
...
2. Create a test user account for yourself. Create a group called `admins` and assign the test account to that group.
...
First, git clone / fork the template repo:

```{sh}
git clone https://github.com/presstofan/shinyproxy-docker-compose-demo.git
```
...
proxy:
  ...
  heartbeat-rate: 10000 # the user's browser will sent a heartbeat call every heartbeat-rate milliseconds (10s as default)
  ...
  usage-stats-url: {url_to_your_InfluxDB_instance}:8086/write?db=shinyproxy_usagestats # use InfluxDB to store usage statistics; can be in a different server
  # usage-stats-username: xxxx # influxdb username
  # usage-stats-password: xxxx # influxdb password
  ...
  openid:
    ...
    auth-url: # https://{cognito_domain}.{region}.amazoncognito.com/oauth2/authorize
    token-url: # https://{cognito_domain}.{region}.amazoncognito.com/oauth2/token
    jwks-url: # https://cognito-idp.{region}.amazonaws.com/{userPoolId}/.well-known/jwks.json
    logout-url: # https://{cognito_domain}.{region}.amazoncognito.com/logout?client_id={client_id}&logout_uri={host_url}
...
1. The default `port` for ShinyProxy is 8080 and we use it. If this is changed, the docker-compose files below needs to be updated accordingly.
2. `usage-stats-url` need to updated to point to the AWS EC2 instance hosting the usage statistics tools. The ':8086/write?db=shinyproxy_usagestats' bit is important as it specify the port of the InfluxDB and the name of the database (which we will create later).
...
You will need to change all the instances of `app.example.com` (4 in total) to your own domain name and that's it! With this file, Nginx will listen port 80 (HTTP) for challenge (an important step for setting up the SSL certificate) and redirect traffic to port 443 (HTTPS), which has the certificate. Then the traffic will be directed to port 8080 of the local `shinyproxy` service where the magic happens. Nginx set up is fairly complicated and you can check the [official guide](https://www.nginx.com/resources/wiki/start/topics/examples/full/) for details.
...
1. We need to link the Certbot and Nginx together by using two shared volumes `./data/certbot/conf:/etc/letsencrypt` and `./data/certbot/www:/var/www/certbot`. In this way, when Certbot obtains the certificate, Nginx is able to access it.
2. The code after `entrypoint` under the Certbot service is to make sure Certbot check if your certificate is up for renewal every 12 hours as recommended by Let’s Encrypt.
3. The code after `command` under the Nginx service is to make that Nginx reloads the newly obtained certificates every 6 hours.
4. We need to instruct docker-compose to create a container network called `sp-net` that links all the services together.
...
In this shell script, you will need to change 'app.example.com' with your own domain name and enter your email address, so Let's Encrypt can contact you if you certificate is about to expire or has any issue.
...
SSH to the `ShinyProxy Machine` and git pull your own configuration files (`shinyproxy-docker-compose-demo`). The structure of the files and directory should remain the same. You should already have the Docker Engine and Docker Compose installed. Start the Docker service:
...
Setting up the usage statistics monitoring tools is relatively straightforward and can be done without scheduling down time of your `ShinyProxy Machine`.

### Step 1: Setting up `Telegraph` on `ShinyProxy Machine`

ShinyProxy has a built-in function that sends the basic usage information (log-in, sign-out, start/stop apps etc.) to InfluxDB. However, we also need to collect metrics (such as memory and CPU usage) from the host machine and Docker Engine. To achieve this, we will need to use [Telegraf](https://www.influxdata.com/time-series-platform/telegraf/).

Again, we need to SSH to `ShinyProxy Machine` and set up a Telegraf user. It makes it easier to set up permissions for future configuration.

```{sh}
sudo useradd -rs /bin/false telegraf
```

Then create a new folder called `telegraf` in `etc` directory.
...
Similar to other services, `telegraph` need a configuration file to work properly and it can be quite complicated. Fortunately, we can create a template configuration file using the `telegraph`'s Docker image.

```{sh}
sudo docker run --rm telegraf telegraf config | sudo tee /etc/telegraf/telegraf.conf &lt; /dev/null
```

And then we give the `telegraf.conf` the correct permission settings to make sure no one can change it except for the Telegraf user and root user.

```{sh}
sudo chown telegraf:telegraf /etc/telegraf/*
```

Now, time to test your vim skill. We need to open the `telegraf.conf` and do two things. First, locate the '[[outputs.influxdb]]' section and update the `urls` to the ip/domain address of the `Usage Monitor Machine`. Don't forget the specify the port 8086.

```{sh}
urls = ["http://{domain_name}:8086"]
```

Then locate the 'HTTP Basic Auth' section and modify the credentials accordingly if you do plan to set up credentials.
...
Finally, locate the '[[inputs.docker]]' section and uncomment the block and set it up with the following settings:
...
Typically, telegraf must be given permission to access the docker daemon unix socket when using the default endpoint. This can be done by adding the telegraf unix user to the docker unix group with the following command:
...
```{sh}
sudo docker run -d --user 998:998 --name=telegraf \
